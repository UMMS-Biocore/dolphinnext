...
//    }
    ...
    function updateAmzInst(){
        ...
        if (!count($data) > 0){ 
            //replace return with write log to file.
            return "$time There is no instance to trigger autoshutdown."; 
        } else {
            $dbfun = new dbfuncs();
            foreach ($data as $profileData):
            $ownerID = $profileData["owner_id"];
            ...
            //replace return with write log to file.
            return "$time profileId:$profileId status:$profileStatus shutdownLog:$triggerShutdown\n";
            endforeach;
        }
    }
    ...
    //http://localhost:8080/dolphinnext/api/service.php?upd=updateStatus
    function updateStatus (){
        ...
        $data=$this->queryTable($sql);
        $time = date("M-d-Y H:i:s");
        if (!count($data) > 0){ return "$time Active run is not found."; }
        $dbfun = new dbfuncs();
        ...
        return "$time runId:$project_pipeline_id status:$finalRunStatus\n";
        //replace return with write log to file.
        endforeach;
    }
}
